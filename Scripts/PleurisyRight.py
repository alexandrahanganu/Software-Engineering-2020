...
def def_labels_right_has_pleurisy(nume_pacient):
...
def create_train_data_for_tbc():
    ...
    for sub in subfolders:
        print(sub[-3:])
        label=def_labels_right_has_pleurisy(sub[-3:])
    ...
    np.save('train_data_right_PLE.npy',training_data)
...
if path.exists('train_data_right_PLE.npy') is False: #if train_data doesn't exist / generate the file
    train_data=create_train_data_for_tbc()
else:
    train_data=np.load('train_data_right_PLE.npy',allow_pickle=True) #else just load it
...
if path.exists('model_right_PLE.h5') is False: # check if model is already trained, if not train it
    model.compile(optimizer='adam',loss='categorical_crossentropy',metrics=['accuracy'])
    model.fit(X,Y,batch_size=200,validation_data=(test_x,test_y),epochs=3)
    model.save('model_right_PLE.h5')
else:
    model.load_weights('model_right_PLE.h5')
...
with open('Pleurisy_Results_Right.csv','w') as f:
    f.write('id, percentage\n')

with open('Pleurisy_Results_Right.csv','a') as f:
